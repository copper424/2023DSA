//https://leetcode.cn/problems/cat-and-mouse-ii/solutions/1473725/mao-he-lao-shu-ii-by-leetcode-solution-e5io/
		...
		bool canMouseWin(vector<string> grid, int catJump, int mouseJump) {
			...
			for (int mouse = 0; mouse < total; mouse++) {
				...
				for (int cat = 0; cat < total; cat++) {
					...
					for (auto &dir : dirs) {
						for (int row = mouseRow + dir[0], col = mouseCol + dir[1], jump = 1; row >= 0 && row < rows && col >= 0 && col < cols
						        && grid[row][col] != '#' && jump <= mouseJump; row += dir[0], col += dir[1], jump++) {
							int nextMouse = getPos(row, col), nextCat = getPos(catRow, catCol);
							degrees[nextMouse][nextCat][MOUSE_TURN]++;
						}
						for (int row = catRow + dir[0], col = catCol + dir[1], jump = 1; row >= 0 && row < rows && col >= 0 && col < cols
						        && grid[row][col] != '#' && jump <= catJump; row += dir[0], col += dir[1], jump++) {
...
int main() {
	...
	Solution solution;
	if (solution.canMouseWin(grid, catJump, mouseJump)) {
		cout << "true";
	} else {
		cout << "false";
	}
